...
// Background image processing (non-blocking)
async function processImageInBackground(product_image, product_category, scene_description, price_overlay, decryptedBody) {
   ...
   try {
       ...
       if (toPhone) {
           const caption = (price_overlay && price_overlay.trim())
               ? `✨ Your generated ${product_category.trim()} image — ${price_overlay.trim()}`
               : `✨ Your generated ${product_category.trim()} image is ready!`;
           
           await sendWhatsAppImageMessage(toPhone, imageUrl, caption);
           console.log('✅ Background processing completed and image sent');
       } else {
           console.warn('Phone number not found; could not send WhatsApp message');
       }
       ...
       if (toPhone) {
           try {
               await sendWhatsAppImageMessage(
                   toPhone, 
                   null,
                   `❌ Sorry, there was an error generating your ${product_category} image. Please try again.`
               );
           } catch (sendError) {
...
async function handleDataExchange(decryptedBody) {
 ...
 if (action === 'data_exchange') {
   ...
   if (data && typeof data === 'object') {
     ...
     // Process image generation in background (don't await)
     processImageInBackground(product_image, product_category, scene_description, price_overlay, decryptedBody)
         .catch(error => {
     ...
     return { 
         ...
         data: { 
             processing_message: "Your image is being generated and will be sent to you shortly via WhatsApp..." 
         } 
     };
   } else {
